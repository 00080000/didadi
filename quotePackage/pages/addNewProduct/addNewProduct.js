Page({
    ...
    // 2. 获取组合商品列表（添加productCode字段和详情获取）
          ...
          success: (res) => {
            if (res.statusCode === 200 && res.data.code === 200) {
              ...
              updatedCombinations[index] = {
                ...
                products: products,
                // 重新计算组合商品总价
                price: products.reduce((sum, item) => sum + (item.unitPrice * item.quantity), 0)
              };
    ...
    confirm() {
      ...
      const selected = [
        ...this.data.singleProduct
          ...
          .map(item => ({ ...item, type: 'singleProduct', productId: item.id })),
        ...
        ...this.data.temporaryProduct
          .filter(item => item.select)
          .map(item => ({ ...item, type: 'customProduct', productId: item.id }))
      ];
      ...
      console.log('已添加到全局selectedProducts，总数:', app.globalData.selectedProducts.length);
